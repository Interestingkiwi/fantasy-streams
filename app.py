...
def download_debug_dump(filename):
    """
    Helper route to download debug JSON files generated by the worker.
    Only allows downloading specific safe files from the static directory.
    """
    # Security: Ensure user is logged in
    if 'yahoo_token' not in session:
         return jsonify({'error': 'Not authenticated'}), 401

    # Security: Whitelist allowed filenames to prevent accessing system files
