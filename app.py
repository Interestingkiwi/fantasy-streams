...
def _enrich_goalie_data(cursor, players_list):
    ...
    for p in goalies:
        # 1. Get Basic Info
        # Note: 'team' might be 'player_team' depending on source
        team = p.get('team') or p.get('player_team')
        pid = p.get('player_id') or p.get('nhlplayerid')
        ...
        if last_played_date and next_game:
            ...
            # Standard "Days Rest" is diff - 1 (Play Mon, Play Wed = 1 Day Rest)
            # If diff is 1 (Back to Back), Rest is 0.
        ...
        # 5. Fetch Split Stats (Rest & Location)
        # 5a. Rest Split (Cap at 4 for DB lookup)
        rest_w_pct = "N/A"
        rest_sv_pct = "N/A"
        ...
        if next_loc in ['H', 'A']:
            db_loc_val = 'H' if next_loc == 'H' else 'R' # DB uses 'R' for Road
            cursor.execute("""
                SELECT SUM(wins) as w, COUNT(*) as gp, SUM(saves) as sv, SUM(shotsagainst) as sa
                FROM historical_goalie_stats
        ...
        cursor.execute("""
            SELECT startpct FROM goalie_to_date WHERE nhlplayerid = %s
        """, (pid,))
